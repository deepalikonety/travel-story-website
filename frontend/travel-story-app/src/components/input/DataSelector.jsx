...
const DataSelector = ({date,setDate}) => {
  ...
  return (
    <div>
      <button className='inline-flex items-center gap-2 text-[13px] font-medium text-sky-600 bg-cyan-100/40 hover:bg-sky-200/70 rounded px-2 py-1 cursor-pointer ' onClick={()=>{
      ...
      {openDatePicker && <div className='overflow-y-scroll p-5 bg-sky-50/80 rounded-lg relative pt-9'>
        ...
        <DayPicker 
        captionLayout='dropdown-buttons' mode="single"
        selected={date} 
        onSelect={setDate} 
        pagedNavigation />
      </div>}
